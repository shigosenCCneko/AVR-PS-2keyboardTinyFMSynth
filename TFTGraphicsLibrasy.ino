...
//  digitalWrite(dc, HIGH);
  asm("sbi 5,0");
  TftData(d1>>8); TftData(d1); TftData(d2>>8); TftData(d2);
}
...
void InitDisplay () {
  ...
  Command(0x3A); TftData(0x55);               // Set color mode - 16-bit color
  delay(10);
  Command(0x36);TftData(0);
  
  Command(0x20+invert);                    // Invert
//  Command(0x36); TftData(rotate<<5);          // Set orientation

  Command(0x2A);TftData(4);
  TftData(0);TftData(0);
  TftData(0);TftData(0xf0);

  Command(0x2b);TftData(4);
  TftData(0);TftData(0);
  TftData(0);TftData(0xf0);

//  Command(0x36); TftData(0b01100000);          // Set orientation
    Command(0x36); TftData(0b01100000);          // Set orientation
...
void ClearDisplay () {
  ...
  Command(0x3A); TftData(0x03);               // 12-bit colour
  Command(RAMWR);
  ...
  Command(0x3A); TftData(0x05);               // Back to 16-bit colour
}
...
//  if(x < 240){
  ...
  Command(RAMWR); TftData(fore>>8); TftData(fore & 0xff);
//  }
}
...
void FillRect (int w, int h) {
  ...
  for (int p=0; p<w*h*2; p++) {
    TftData(fore>>8); TftData(fore & 0xff);
  }
}
    ...
    for (int xr=0; xr<scale; xr++) {
      for (int yy=0; yy<8; yy++) {
        ...
        for (int yr=0; yr<scale; yr++) {
          TftData(colour>>8); TftData(colour & 0xFF);
        }
      }
    }
  }
