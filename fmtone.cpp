...
#define TLSHIFT 8
#define SULSHIFT 4
//uint8_t envelope_cnt[16]  = {0, 1, 2, 3, 4, 5, 7, 10, 13, 20, 29, 43, 64, 86, 128, 255};
...
//extern  char wave_sin[WAVE_TBL_SIZE];
//extern  char wave_tri[WAVE_TBL_SIZE];
//extern  char  wave_saw[WAVE_TBL_SIZE];
//extern  char  wave_rect[WAVE_TBL_SIZE];
//extern char  wave_clipsin[WAVE_TBL_SIZE];
//extern  char  wave_abssin[WAVE_TBL_SIZE];
//extern char  wave_rnd[WAVE_TBL_SIZE];
//extern  char  wave_user[WAVE_TBL_SIZE];

extern char wave_mul31sin[WAVE_TBL_SIZE];
extern  char wave_mul31tri[WAVE_TBL_SIZE];
extern  char  wave_mul31saw[WAVE_TBL_SIZE];
extern  char  wave_mul31rect[WAVE_TBL_SIZE];
extern char  wave_mul31clipsin[WAVE_TBL_SIZE];
extern  char  wave_mul31abssin[WAVE_TBL_SIZE];
extern char  wave_mul31rnd[WAVE_TBL_SIZE];
extern  char  wave_mul31user[WAVE_TBL_SIZE];

FmTone::FmTone() {
  for (int i = 0; i < MAX_MIDITONE * 2 ; i ++) {
    fm_operator[i].wave_tbl = (wave_mul31tri);
...
FmTone::set_envelope(uint8_t ch, uint8_t atk, uint8_t decy, uint8_t sul, uint8_t sus, uint8_t rel, uint8_t tl, uint8_t mul) {
  ...
  if(sul == 15){
    ...
    fm_operator[ch].sul = (31 - sul * 2)*SULSHIFT;
  }
  fm_operator[ch].sus = conv_count(sus);
  fm_operator[ch].rel = conv_count(rel);
  fm_operator[ch].tl =  (31 - tl) * TLSHIFT;
      ...
      switch (com) {
        ...
        case MIDI_NOTE_ON:
          if (dat3 == 0) {  //ベロシティ0はノートオフ
            ...
            fm_operator[voice_ch * 2 + 1].tl = (  (dat3 >> 2) * TLSHIFT);
    ...
    switch (com) {
      ...
      case MIDI_NOTE_ON:
        ...
        if (dat3 == 0) {
          ...
          fm_operator[ch * 2 + 1].tl = (  (dat3 >> 2) *TLSHIFT );
        }
        break;
...
FmTone::change_sul(uint8_t ch,uint8_t sul) {
  fm_operator[ch].sul  = (sul*2 ) *SULSHIFT;
}
